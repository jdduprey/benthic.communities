...
algae_data <- filter_get_PA_data(species.by.sample.alltax, ben_algae_list)
...
ggplot(raw.pca.to.plot, aes(x=jaccard_MDS1, y=jaccard_MDS2)) +
  geom_point(size=3, aes(color=factor(site))) +  # shape=factor())
  theme_bw() +
  labs(x="PC1",y="PC2", color="Site") +
  ggtitle('COI - Jaccard PA')  # + geom_text(aes(label=sample))

ggsave("../figures/nonconverg_inv_eily_PCA.png")
dev.off()
