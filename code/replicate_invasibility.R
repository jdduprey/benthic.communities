...
sum(by_rep_hash$nReads)

by_rep_species <- by_rep_hash %>%
  group_by(sample, species) %>%
  summarize(nReads=sum(nReads))
write.csv(by_rep_species,"../data/by_rep_species.csv")

sum(by_rep_species$nReads)

# ====================================================
# now lets just repeat the data exploration and visualization code with the rep data
# ====================================================
...
QA <- unique(by_rep_species$species)[!(unique(by_rep_species$species)) 
                                        %in% nonnative_vec$species]
print(QA)

QA <- nonnative_vec$species[!(nonnative_vec$species
                              %in% unique(by_rep_species$species))]
print(QA)

unique(by_rep_species$species)

# ====================================================
species_annotated <- left_join(nonnative_vec, species_annotated)
# now use code from benthic boxplots 
species_by_rep_alltax <- left_join(by_rep_species, species_annotated, by='species')
species_by_rep_alltax <- species_by_rep_alltax %>%
...
total_nn_detections <- total_nn_detections %>%
  separate(sample, into = c("site","date" ), sep = "_", remove = F) %>%
  separate(date, into = c("year", "month"), sep = 4) %>%
  separate(month, into =c("month", "rep"), sep = 2) %>%
  mutate(
    region = case_when(
...
ggplot(nonnative_vs_all_species, aes(x=all_sp_detections, y=n_detections, color=site)) +
  ...
  theme_classic()
ggsave(filename="../figures/reps_separate/richness_ratio_by_site.png")
...
ggsave(filename="../figures/reps_separate/SE_richness.png")
...
ggplot(SD_bar_plt, aes(x=mean_all_sp, y=mean_detections, color=site)) +
  ...
  geom_errorbarh(aes(xmin=xmin,xmax=xmax)) +
  scale_y_continuous(breaks=0:7)
ggsave(filename="../figures/reps_separate/SD_richness.png")
...
ggplot(nonnative_vs_all_species, aes(x=all_sp_detections, y=n_detections, color=region)) +
  ...
  geom_point()
ggsave(filename="../figures/reps_separate/richness_ratio_by_region.png")
...
ggplot(nonnative_vs_all_species, aes(x=all_sp_detections, y=n_detections, color=month)) +
  ...
  theme_classic()
ggsave(filename="../figures/reps_separate/richness_ratio_by_month.png")
...
ggsave(filename="../figures/reps_separate/richness_ratio_by_season.png")
...
ggsave(filename="../figures/reps_separate/probable_nonnative_richness.png")
...
ggplot(nonnative_vs_all_species, aes(x=site, y=prop_nn)) + 
  ...
  geom_boxplot() +
  theme_classic()
ggsave(filename="../figures/reps_separate/proportion_probable_nn.png")
...
ggplot(total_nn_detections, aes(x=region, y=n_detections)) + 
  ...
  geom_boxplot()
ggsave(filename="../figures/reps_separate/region_probable_nonn_richness.png")
...
#TODO STOPPED EDITING HERE FOR THE replicate script
...
ggplot(enviro_plot_df, aes(x = Salinity, y = prop_nn, color = site)) +
  ...
  theme_classic()
ggsave(filename="../figures/reps_separate/nn_prop_salinity.png")
